...
class BFGS_With_Trust_Region(object):
    ...
    def __init__(self, seed=None, epsilon=1.e-4, maxIter=300, init_point=None, func_idx=1):
        ...
        self.__fPath = list()                       # Store the path of values of the target function
    ...
    def solve(self):
        ...
        xCurr = self.__get_init_point()             # Initialize the first point of optimization
        fCurr = self.__targetFunction.func(xCurr[0, 0], xCurr[1, 0])      # Calculate the target function value of the first point
        gCurr = self.__targetFunction.grad(xCurr[0, 0], xCurr[1, 0])      # Calculate the derivative value of the first point
        BCurr = self.__init_B(xCurr.shape[0])       # Initialize the matrix D with Identity Matrix
        ...
        for i in range(self.__maxIter):
            ...
            # Implementation of BFGS
            dCurr = -np.matmul(BCurr, gCurr)        # The current direction of optimization
            alpha = self.__calc_alpha_by_ArmijoRule(
                xCurr, fCurr, gCurr, dCurr)         # Calculate the alpha by the Armijo Rule
            s_k = alpha * dCurr
            flag = False
            ...
            m_xk_sk = self.__targetFunction.func(xCurr[0], xCurr[1]) + np.dot(s_k.T, self.__targetFunction.grad(xCurr[0,0], xCurr[1,0])) + 1/2*np.dot(np.dot(s_k.T, np.linalg.inv(BCurr)), s_k)
            m_xk = self.__targetFunction.func(xCurr[0], xCurr[1])
            rho = (self.__targetFunction.func(xCurr[0,0], xCurr[1,0]) - self.__targetFunction.func(xCurr[0,0]+s_k[0,0], xCurr[1,0]+s_k[1,0])) / (m_xk - m_xk_sk)
            # Update the radius of trust region
            deltaNext = -1
            if rho < 1/4:
                deltaNext = 1/4*deltaCurr
            else:
                if flag == True or (rho > 3/4 and np.linalg.norm(s_k) == deltaCurr):
            ...
            if rho > self.__eta:
                DNext = self.__update_B_by_BFGS(xCurr, gCurr, xNext, gNext, BCurr)
            else:
                DNext = BCurr
            ...
            if self.__is_converged(gCurr):
                ...
                print("BFGS_With_Trust_Region not converged after {} steps!".format(self.__maxIter))
    ...
    def show(self):
        ...
        for i in range(len(self.__fPath)):
            ...
            x2 = x2Path[i]
            # (j*0.1+0.1, j*0.1+0.1, j*0.1+0.1, j*0.1+0.1)
            circle = plt.Circle((x1, x2), r, color=[round(random.uniform(0.5, 1), 1) for _ in range(3)], fill=True)
            plt.gcf().gca().add_artist(circle)
    ...
    def __calc_alpha_by_ArmijoRule(self, xCurr, fCurr, gCurr, dCurr, c=1.e-4, v=0.5):
        ...
        while True:
            if fNext <= fCurr + c * alpha * np.matmul(dCurr.T, gCurr)[0, 0]: break
            i += 1
            alpha = v ** i
            xNext = xCurr + alpha * dCurr
            fNext = self.__targetFunction.func(xNext[0, 0], xNext[1, 0])
...
if __name__ == '__main__':
    obj = BFGS_With_Trust_Region(init_point=[[0.3], [0.4]])
